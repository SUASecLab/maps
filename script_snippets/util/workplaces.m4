        ...
        `() => {
            XHR(`GET', `"/extensions/getNoVNCPassword/?token=" + WA.player.userRoomToken',`function() {
                    ...
                    TIMEOUT(`async function() {
                        coWebsite = OPEN_COWEBSITE(`"/websockify/vnc.html?path=" + machine + "&autoconnect=true&resize=scale&password=" + password + "&service=noVNC&token=" + WA.player.userRoomToken', `false', `"fullscreen"')
                        if (machine2) {
                            coWebsite2 = OPEN_COWEBSITE(`"/websockify/vnc.html?path=" + machine2 + "&autoconnect=true&resize=scale&password=" + password + "&service=noVNC&token=" + WA.player.userRoomToken', `false', `"fullscreen"')
                        }
